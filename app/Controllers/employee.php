...
';
            ...
            if ($respuesta == 1) {
                ...
                $this->session->setFlashdata('flag', [
                    'type' => 'success',
                    'msg' => $nickename . $EmployeeModel->getInsertID() . ' ' . 'Se
registro correctamente'
    ...
    public function guardarEditar()
    {
        ...
        if ($resultado > 0) {
            $this->session->setFlashdata('flag', [
                'type' => 'success',
                'msg' => 'Se a guardado satisfactoriamente
' . $id . $this->request->getVar('nombre') . '!'
    ...
    public function delete($id)
    {
        ...
        if ($resultado > 0) {
            ...
            $this->session->setFlashdata('flag', [
                'type' => 'danger',
                'msg' => 'Error, no se pudo eliminar, contactar al
administrador'
            ]);
    ...
    <DIV>Hola ' . $data['nickename'] . ' , por favor inglesa la contraseña que se muestra mas adelante con este mismo
        correo.</DIV>
    <br>
    ...
    ';
            ...
            if ($respuesta == 1) {
                ...
                $this->session->setFlashdata('flag', [
                    'type' => 'success',
                    'msg' => $data['nickename'] . ', ' . 'Se mando la nueva
    clave al usuario'
                ]);
                return redirect()->to('employee');
                ...
                $this->session->setFlashdata('flag', [
                    'type' => 'danger',
                    'msg' => 'No se mando la clave por email, vuelve a
    restablecerlo por favor.'
                ]);
                return redirect()->to('employee');
            }
        }
    ...
    public function concentradoempleado($nickname)
    {
        $session = \Config\Services::session();
        $WorklistModel = new \App\Models\WorklistModel();

        $data['worklist'] = $WorklistModel->where('nameEmployee', $nickname)->findAll();

        return view('employee/concentradoempleado', $data);
